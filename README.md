...
### 4. 단일 exe 빌드의 네이티브 크래시

배포는 단일 exe로 하기로 했는데, PyInstaller 빌드에서 두 번 크래시를 만났습니다.
...
받은 뒤 루트의 `start.bat`을 더블클릭하면 됩니다. 소스가 바뀌었으면 자동으로 빌드하고,
아니면 마지막 빌드를 실행한 뒤 서버를 띄우고 브라우저(http://localhost:8765)를 엽니다.
첫 실행은 환경 준비와 빌드로 시간이 걸릴 수 있고, `uv`가 설치돼 있어야 합니다.
...
명시적으로 선택해야 했고, 단일 실행본으로 패키징하기까지 네이티브 레벨의 크래시를
추적해 해결해야 했습니다. 라이브러리를 쓰는 데서 멈추지 않고, 실제로 쓰이는 도구가 갖춰야

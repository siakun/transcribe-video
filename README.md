...
로컬 GPU로 동영상에서 텍스트와 자막을 추출하는 도구입니다. 상용 STT의 비용과 업로드
제약, 한국어 환각을 피해 내 PC GPU만으로 강의 영상 수십 개를 한 번에 글로 옮기고, 그
결과를 요약이나 지식화(LLM Wiki)의 입력으로 활용합니다.

Whisper 음성인식 모델을 빠르게 돌리는 추론 라이브러리인 faster-whisper로 전사하고,
추가로 한국어에서 자주 나오는 Whisper 환각을 걸러내는 다층 후처리와 유튜브 다운로드
전사 파이프라인을 직접 설계해 붙였습니다.
...
싶었습니다. 마침 로컬 LLM과 개인 GPU 성능이 올라오고, 프론티어 LLM이 긴
...
품질을 직접 손볼 수 있고, 일괄 처리와 이어하기, 자막 없는 유튜브 영상까지 되는
도구입니다. 핵심은
